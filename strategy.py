...
def fetch_market_data_with_retry(symbol: str, timeframe: str, limit: int = 30, max_retries: int = 3) -> list:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if DEBUG_MODE:
                print(f"[DATA] Successfully fetched {len(data)} {timeframe} candles for {symbol}")
            return data
        except ccxt.NetworkError as e:
            print(f"[RETRY] Network error (attempt {attempt + 1}/{max_retries}): {str(e)}")
...
def calculate_bias(spot_symbol: str) -> tuple[float, float]:
    ...
    """
    try:
        daily_data = fetch_market_data_with_retry(spot_symbol, DAILY_TIMEFRAME, limit=CANDLES_LOOKBACK)
        
        if len(daily_data) < SMA_PERIOD:
            print(f"[WARNING] Insufficient daily candles ({len(daily_data)}) for SMA calculation")
            return 0.0, 0.0
        ...
        if DEBUG_MODE:
            print(f"[BIAS] Current Price: {current_price:.2f}, SMA20: {sma20:.2f}, BIAS%: {bias_pct:.4f}%")
...
def calculate_spread(spot_symbol: str, futures_symbol: str) -> tuple[float, float, float]:
    ...
    """
    try:
        spot_data = fetch_market_data_with_retry(spot_symbol, HOURLY_TIMEFRAME, limit=2)
        futures_data = fetch_market_data_with_retry(futures_symbol, HOURLY_TIMEFRAME, limit=2)
        ...
        if DEBUG_MODE:
            print(f"[SPREAD] Spot: {p_spot:.2f}, Futures: {p_futures:.2f}, Spread%: {spread_pct:.4f}%")
        
        return spread_pct, p_spot, p_futures
    
    except Exception as e:
        print(f"[ERROR] Failed to calculate Spread: {str(e)}")
        send_telegram_alert(f"Spread calculation error: {str(e)}", is_error=True)
        return 0.0, 0.0, 0.0
...
def validate_api_credentials() -> bool:
    ...
    if not GATE_API_KEY or not GATE_API_SECRET:
        print("[ERROR] Gate.io API credentials not configured")
...
def run_trading_cycle() -> None:
    ...
    try:
        ...
        spread_pct, spot_price_hourly, futures_price = calculate_spread(spot_symbol, futures_symbol)
        ...
        # Log metrics
        print(f"\n[METRICS]")
        print(f"  BIAS%:           {bias_pct:>8.4f}% (threshold: {BIAS_THRESHOLD}%)")
        print(f"  Spread%:         {spread_pct:>8.4f}% (threshold: {SPREAD_THRESHOLD}%)")
        ...
        bottom_entry_cond = (bias_pct < BIAS_THRESHOLD) and (spread_pct < SPREAD_THRESHOLD)
        exit_cond = (bias_pct > 0)
        
        if bottom_entry_cond and current_position < MAX_POSITION_SIZE:
            # Entry signal
            buy_amount = min(CONTRACT_SIZE, MAX_POSITION_SIZE - current_position)
            print(f"\n[SIGNAL] Bottom Entry triggered!")
            print(f"  Condition: BIAS% ({bias_pct:.4f}%) < {BIAS_THRESHOLD}% AND Spread% ({spread_pct:.4f}%) < {SPREAD_THRESHOLD}%")
